...
public class ChargeModPlayer : ModPlayer
{
	...
	public StatModifier GetChargeAmountModifier()
	{
		...
		if (Impatience)
			chargeAmount -= ImpatiencePotion.ChargeLevelDecrease / 100f;
	...
	public int GetMaxCharge()
	{
		...
		else if (HasCarBattery)
			maxCharge += CarBattery.maxChargeIncrease / 100f;
		...
		if (Player.HeldItem.ModItem is ChargeWeapon weapon && weapon.blowWeapon) {
			if (HasBreathingAid)
				...
				maxCharge += IronLung.MaxChargeIncrease / 100f;
		}

		if (Stamina)
			maxCharge += StaminaPotion.MaxChargeIncrease / 100f;
	...
	public override float UseTimeMultiplier(Item item)
	{
		if (item.ModItem is ChargeWeapon weapon) {
			...
			if (HasGenerator)
				speed += Generator.ChargeSpeedIncrease / 100f;
			...
			if (Charge)
				speed += ChargePotion.ChargeSpeedIncrease / 100f;
	...
	public void PostProjectileEffects(bool blowWeapon, int charge, int chargeLevel, Projectile proj, ChargerProjectile chargerProj)
	{
		...
		if (HasChargeRepository || HasGenerator)
			chargerProj.Repository = charge;
		if (LeatherGlove)
	...
	public void PostProjectileEffects(Projectile proj, ChargerProjectile chargerProj)
	{
		...
		if (MADChest)
			chargerProj.BossBonus = true;
		if (FestiveSet)
			chargerProj.SpawnRockets = true;
	}
