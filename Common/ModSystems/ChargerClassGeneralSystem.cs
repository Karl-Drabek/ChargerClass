...
{
	public static int CopperBarRecipeGroup,
		SilverBarRecipeGroup,
		HardmodeOreBlowguns,
		IchorCannisters,
		Rockets,
		CopperOreRecipeGroup,
		GoldBarRecipeGroup,
		TitaniumBarRecipeGroup,
		ShadowScaleRecipeGroup,
		DemoniteBarRecipeGroup,
		VoltaicScrapRecipeGroup;
	public static ChargerClassGeneralSystem Instance =
		ModContent.GetInstance<ChargerClassGeneralSystem>();
	...
	public static ModKeybind AdamantiteBlowgunDamageToggle { get; private set; }
	...
	public override void AddRecipeGroups()
	{
		...
		RecipeGroup copperBarRecipeGroup = new RecipeGroup(
			() =>
				$"{Language.GetTextValue("LegacyMisc.37")} {Lang.GetItemNameValue(ItemID.CopperBar)}",
			ItemID.CopperBar,
			ItemID.TinBar
		);
		CopperBarRecipeGroup = RecipeGroup.RegisterGroup(
			nameof(ItemID.CopperBar),
			copperBarRecipeGroup
		);

		RecipeGroup copperOreRecipeGroup = new RecipeGroup(
			() =>
				$"{Language.GetTextValue("LegacyMisc.37")} {Lang.GetItemNameValue(ItemID.CopperOre)}",
			ItemID.CopperOre,
			ItemID.TinOre
		);
		CopperOreRecipeGroup = RecipeGroup.RegisterGroup(
			nameof(ItemID.CopperOre),
			copperOreRecipeGroup
		);

		RecipeGroup rockets = new RecipeGroup(
			() =>
				$"{Language.GetTextValue("LegacyMisc.37")} {Lang.GetItemNameValue(ItemID.RocketI)}",
			ItemID.RocketI,
			ItemID.RocketII,
			ItemID.RocketIII,
			ItemID.RocketIV
		...
		SilverBarRecipeGroup = RecipeGroup.RegisterGroup(
			nameof(ItemID.SilverBar),
			silverBarRecipeGroup
		);

		RecipeGroup goldBarRecipeGroup = new RecipeGroup(
			() =>
		...
		RecipeGroup titaniumBarRecipeGroup = new RecipeGroup(
			() =>
				$"{Language.GetTextValue("LegacyMisc.37")} {Lang.GetItemNameValue(ItemID.TitaniumBar)}",
			ItemID.TitaniumBar,
			ItemID.AdamantiteBar
		);
		TitaniumBarRecipeGroup = RecipeGroup.RegisterGroup(
			nameof(ItemID.TitaniumBar),
			titaniumBarRecipeGroup
		);

		RecipeGroup shadowScaleRecipeGroup = new RecipeGroup(
			() =>
				$"{Language.GetTextValue("LegacyMisc.37")} {Lang.GetItemNameValue(ItemID.ShadowScale)}",
			ItemID.ShadowScale,
			ItemID.TissueSample
		);
		ShadowScaleRecipeGroup = RecipeGroup.RegisterGroup(
			nameof(ItemID.ShadowScale),
			shadowScaleRecipeGroup
		);

		RecipeGroup demoniteBarRecipeGroup = new RecipeGroup(
			() =>
				$"{Language.GetTextValue("LegacyMisc.37")} {Lang.GetItemNameValue(ItemID.DemoniteBar)}",
			ItemID.DemoniteBar,
			ItemID.CrimtaneBar
		);
		DemoniteBarRecipeGroup = RecipeGroup.RegisterGroup(
			nameof(ItemID.DemoniteBar),
			demoniteBarRecipeGroup
		);

		RecipeGroup voltaicScrapRecipeGroup = new RecipeGroup(
			() =>
				$"{Language.GetTextValue("LegacyMisc.37")} {Lang.GetItemNameValue(ModContent.ItemType<MightyVoltaicScrap>())}",
			ModContent.ItemType<MightyVoltaicScrap>(),
			ModContent.ItemType<FrightfulVoltaicScrap>(),
			ModContent.ItemType<OpticVoltaicScrap>()
		);
		VoltaicScrapRecipeGroup = RecipeGroup.RegisterGroup(
			"ChargerClass:VoltaicScrapGroup",
			voltaicScrapRecipeGroup
		);

		RecipeGroup hardmodeOreBlowguns = new RecipeGroup(
			() =>
				$"{Language.GetTextValue("LegacyMisc.37")} {Lang.GetItemNameValue(ModContent.ItemType<TitaniumBlowgun>())}",
			ModContent.ItemType<TitaniumBlowgun>(),
			ModContent.ItemType<AdamantiteBlowgun>(),
			ModContent.ItemType<MythrilBlowgun>(),
			ModContent.ItemType<OrichalcumBlowgun>(),
			ModContent.ItemType<CobaltBlowgun>(),
			ModContent.ItemType<PalladiumBlowgun>()
		);
		HardmodeOreBlowguns = RecipeGroup.RegisterGroup(
			"ChargerClass:HardmodeBlowgunGroup",
			hardmodeOreBlowguns
		);

		RecipeGroup ichorCannisters = new RecipeGroup(
			() =>
				$"{Language.GetTextValue("LegacyMisc.37")} {Lang.GetItemNameValue(ModContent.ItemType<IchorCannister>())}",
			ModContent.ItemType<CursedCannister>(),
			ModContent.ItemType<IchorCannister>()
		);
		IchorCannisters = RecipeGroup.RegisterGroup(
			"ChargerClass:IchorCannisters",
			ichorCannisters
		);
	}
		...
		for (int c = 0; c < Main.maxChests; c++) {
			...
			switch (chestTile.TileFrameX) {
				case 1 * 36: //gold chest
					if (WorldGen.genRand.NextBool(2, 3) || ++GoldChestItemCount >= 20)
						continue; //33% chance or already have max items
					for (int i = 0; i < Chest.maxItems; i++) {
						if (chest.item[i].type == ItemID.None) {
							chest
								.item[i]
								.SetDefaults(
									GoldChestItemCount % 2 == 0
										? ModContent.ItemType<TripleShot>()
										: ModContent.ItemType<TripleShot>()
								);
							break;
						}
				...
				case 4 * 36: //shadow chest
					if (WorldGen.genRand.NextBool(0, 20) || ++ShadowChestItemCount >= 10)
						continue; //15% chance or already have max items
					for (int i = 0; i < Chest.maxItems; i++) {
						if (chest.item[i].type == ItemID.None) {
	...
	public override void PostSetupContent()
	{
		...
		ItemID.Sets.ExtractinatorMode[ModContent.ItemType<AncientDebris>()] =
	...
	public override void Load()
	{
		...
		AdamantiteBlowgunDamageToggle = KeybindLoader.RegisterKeybind(
			Mod,
			"AdamantiteBlowgunDamageToggle",
			"K"
		);
	}
	...
	public override void Unload()
	{
		...
		AdamantiteBlowgunDamageToggle = null;
	}
}