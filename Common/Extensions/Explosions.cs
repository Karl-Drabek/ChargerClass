...
{
	public static void ExplodeSquare(
	...
	public static void ExplodeCircle(
		Vector2 position,
		int explosionRadius,
		int damage,
	...
	)
	{
		...
		for (int k = 0; k < Main.maxNPCs; k++)
		{
			...
			if (Vector2.DistanceSquared(target.Center, position) < distanceSquared)
			{
				...
				target.SimpleStrikeNPC(
					damage,
					hitDirection,
					Main.rand.NextBool((int)critChance, 100),
					knockback,
					damageType,
					damageVariation,
					luck,
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (posx * posx + posy * posy < distanceSquared)
			{
				...
				Gore.NewGoreDirect(
					new EntitySource_Parent(source),
					new Vector2(position.X + posx, position.Y + posy),
					default,
					Main.rand.Next(61, 64),
					Main.rand.NextFloat(1f, (explosionRadius / 200) + 1)
				);
				if (ChargerClassConfig.Instance.AudioToggle) {
