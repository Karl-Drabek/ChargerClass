...
public static class Targeting
{
	...
	public static NPC FindClosestLineOfSightNPC(
		Vector2 position,
	...
	)
	{
		...
		{
			NPC target = Main.npc[k];
			if (!Collision.CanHit(position, width1, height1, target.position, wieght2, height))
				continue;
			if (target.CanBeChasedBy())
			{
				float squareDistanceToNPC = Vector2.DistanceSquared(target.Center, position);
	...
	public static NPC FindClosestNPCBiasBoss(Vector2 position, float maxDetectDistance)
	{
		...
		for (int k = 0; k < Main.maxNPCs; k++)
		{
			...
			if (target.CanBeChasedBy())
			{
				if (closestNPC is not null && closestNPC.boss)
				{ //tracking boss and target is not boss
					if (!target.boss)
						continue;
				}
				else
				{ //not tracking boss and new target is boss
					if (target.boss)
						newBoss(Vector2.DistanceSquared(target.Center, position));
				}
				float sqrDistanceToTarget = Vector2.DistanceSquared(target.Center, position);
				if (sqrDistanceToTarget < sqrMaxDetectDistance)
				{
					if (closestNPC is not null && closestNPC.boss)
					{ //tracking boss and target is assumed boss
						newBoss(sqrDistanceToTarget);
					}
					else
					{ //must be tracking not boss and target is also not boss
