...
public class ChargerProjectile : GlobalProjectile
{
	...
	public override void OnHitNPC(Projectile projectile, NPC target, NPC.HitInfo hitInfo, int damage)
	{
		...
		if ((Repository != 0) && hitInfo.Crit)
			Main.player[projectile.owner].GetModPlayer<ChargeModPlayer>().RepositorySuccess(Repository);
	...
	public override void OnKill(Projectile projectile, int timeLeft)
	{
		...
		if (ExplosionSize > 0)
			Explosions.ExplodeCircle(projectile.position, ExplosionSize, 18, ChargerDamageClass.Instance, projectile, knockback: 1f);
		if (Hydrogenized)
			Explosions.ExplodeCircle(projectile.position, 30, projectile.damage / 10, ChargerDamageClass.Instance, projectile);
	...
	public override void ModifyHitNPC(Projectile projectile, NPC target, ref NPC.HitModifiers hitModifiers)
	{
		...
		if (LeatherGloveChargeLevel > 0)
			hitModifiers.CritDamage += LeatherGlove.BonusCritDamage / 100f * LeatherGloveChargeLevel;
