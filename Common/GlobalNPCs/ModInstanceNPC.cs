...
public class ModInstanceNPC : GlobalNPC
{
	...
	{
		if (Dabilitated)
			modifiers.Defense -= npc.defense / 5;
	}
	...
	public override void AI(NPC npc)
	{
		if (mythrilSlow > 0)
		{
			npc.velocity *= 1 - 0.05f * mythrilSlow;
		}
		if (Bound)
			npc.velocity *= 0.95f;
		if (Slimed)
		{
			...
			if (Main.rand.NextBool(4))
				Dust.NewDustPerfect(
					npc.position
						+ new Vector2(
							Main.rand.NextFloat(0, npc.width),
							Main.rand.NextFloat(0, npc.height)
						),
					DustID.Wet,
					Vector2.Zero,
					0,
		...
		if (Stunned)
		{
			...
			Dust dust = Dust.NewDustDirect(npc.position, npc.width, npc.height, DustID.PinkTorch);
			dust.noGravity = true;
			dust.noLight = true;
		...
		if (Plagued)
		{
			...
			for (int k = 0; k < Main.maxNPCs; k++)
			{
				...
				{
					if (Main.rand.NextBool(1, 20 + ((int)distanceToNPC / 100)))
		...
		if (LeadPoisoning)
		{
			...
			{
				Dust dust = Dust.NewDustDirect(npc.position, npc.width, npc.height, DustID.Lead);
				dust.noGravity = true;
			}
		}
		...
		{
			if (Main.rand.NextBool(1, 2))
				Dust.NewDustDirect(npc.position, npc.width, npc.height, DustID.SpookyWood);
			if (Main.rand.NextBool(1, 5))
				Dust.NewDustDirect(npc.position, npc.width, npc.height, DustID.Venom);
		}
	}

	public override void OnKill(NPC npc)
	{
		for (int i = 0; i < OrichalcumEffect / 2; ++i)
		{
			Vector2 velocity = new Vector2(0, Main.rand.Next(6, 8)).RotatedByRandom(
			...
			for (int i = 0; i < 50; i++)
			{
				...
				if (posx * posx + posy * posy < 2500)
				{
					Dust dust = Dust.NewDustPerfect(
						new Vector2(npc.position.X + posx, npc.position.Y + posy),
						54
					);
					dust.scale = Main.rand.NextFloat(1f, 1.3f);
					dust.noGravity = true;
		...
		if (
			IsBunny(npc.type)
			&& !Main.LocalPlayer.HasItem(ModContent.ItemType<SoulofBunnies>())
			&& BunnyKillCount() % 100 == 0
		)
		{
			Item.NewItem(
				...
				ModContent.ItemType<SoulofBunnies>()
			);
		}
	}
	...
	int BunnyKillCount() =>
		Main.BestiaryTracker.Kills.GetKillCount(
			ContentSamples.NpcPersistentIdsByNetIds[NPCID.Bunny]
		)
		+ Main.BestiaryTracker.Kills.GetKillCount(
			ContentSamples.NpcPersistentIdsByNetIds[NPCID.ExplosiveBunny]
		)
		+ Main.BestiaryTracker.Kills.GetKillCount(
			ContentSamples.NpcPersistentIdsByNetIds[NPCID.GoldBunny]
		)
		+ Main.BestiaryTracker.Kills.GetKillCount(
			ContentSamples.NpcPersistentIdsByNetIds[NPCID.BunnySlimed]
		)
		+ Main.BestiaryTracker.Kills.GetKillCount(
			ContentSamples.NpcPersistentIdsByNetIds[NPCID.BunnyXmas]
		)
		+ Main.BestiaryTracker.Kills.GetKillCount(
			ContentSamples.NpcPersistentIdsByNetIds[NPCID.PartyBunny]
		)
		+ Main.BestiaryTracker.Kills.GetKillCount(
			ContentSamples.NpcPersistentIdsByNetIds[NPCID.GemBunnyAmethyst]
		)
		+ Main.BestiaryTracker.Kills.GetKillCount(
			ContentSamples.NpcPersistentIdsByNetIds[NPCID.GemBunnyAmber]
		)
		+ Main.BestiaryTracker.Kills.GetKillCount(
			ContentSamples.NpcPersistentIdsByNetIds[NPCID.GemBunnyDiamond]
		)
		+ Main.BestiaryTracker.Kills.GetKillCount(
			ContentSamples.NpcPersistentIdsByNetIds[NPCID.GemBunnyEmerald]
		)
		+ Main.BestiaryTracker.Kills.GetKillCount(
			ContentSamples.NpcPersistentIdsByNetIds[NPCID.GemBunnyRuby]
		)
		+ Main.BestiaryTracker.Kills.GetKillCount(
			ContentSamples.NpcPersistentIdsByNetIds[NPCID.GemBunnySapphire]
		)
		+ Main.BestiaryTracker.Kills.GetKillCount(
			ContentSamples.NpcPersistentIdsByNetIds[NPCID.GemBunnyTopaz]
		)
		+ Main.BestiaryTracker.Kills.GetKillCount(
			ContentSamples.NpcPersistentIdsByNetIds[NPCID.CorruptBunny]
		)
		+ Main.BestiaryTracker.Kills.GetKillCount(
			ContentSamples.NpcPersistentIdsByNetIds[NPCID.CrimsonBunny]
		);
