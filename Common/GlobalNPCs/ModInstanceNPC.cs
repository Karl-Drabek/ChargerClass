	...
	public override void AI(NPC npc)
	{
		...
		if (Slimed) {
			...
			if (Main.rand.NextBool(1, 4))
				Dust.NewDustPerfect(npc.position + new Vector2(Main.rand.NextFloat(0, npc.width), Main.rand.NextFloat(0, npc.height)), DustID.Wet, Vector2.Zero, 0, Color.LightGreen);
		}
		if (Dabilitated) {
			npc.velocity *= 0.92f;
		...
		if (Plagued) {
			...
			if (Main.rand.NextBool(1, 15))
				Gore.NewGorePerfect(new EntitySource_Death(npc), npc.position, new Vector2(Main.rand.NextFloat(-3f, 3f), Main.rand.NextFloat(-3f, 3f)), 99, Main.rand.NextFloat(0.5f, 0.75f));
			for (int k = 0; k < Main.maxNPCs; k++) {
				NPC target = Main.npc[k];
				if (target.friendly || !target.active || target.dontTakeDamage || target.whoAmI == npc.whoAmI)
					continue;
		...
		if (Cursed) {
			...
			if (Main.rand.NextBool(1, 3))
				dust = Dust.NewDustDirect(npc.position, npc.width, npc.height, DustID.Venom);
		}
	}
		...
		if (Plagued) {
			...
			for (int i = 0; i < 50; i++) {
				...
				if (posx * posx + posy * posy < 2500) {
					...
					dust.noGravity = true;
					if (Main.rand.NextBool(1, 3))
						Gore.NewGorePerfect(new EntitySource_Death(npc), new Vector2(npc.position.X + posx, npc.position.Y + posy), new Vector2(Main.rand.NextFloat(-1f, 1f), Main.rand.NextFloat(-1f, 1f)), 99, Main.rand.NextFloat(0.5f, 0.75f));
				}
			}
		}
