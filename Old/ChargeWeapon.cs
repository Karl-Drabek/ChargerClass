...
public abstract class ChargeWeapon : ModItem
{
	...
	public override void HoldItem(Player player)
	{
		...
		if (player.itemAnimation == player.itemAnimationMax - 1) { //use style for some reason does not update charge. really confusing stuff
			...
			if (player.mouseInterface || !player.HasAmmo(Item)) { //if the player runs out of ammo or hovers over UI
				...
				if (charge == 0)
					return; //exit if not charging
				else if (player.itemAnimation == player.itemAnimationMax - 1) {
					if (SafeCanShoot(player) && (charge > chargeAmount / 4))
						Shoot(player, modPlayer);//shoot the weapon if on first frame and greater than min charge
					else
				...
				if (charge < maxCharge)
					charge += 300 / CombinedHooks.TotalUseTime(Item.useTime, player, Item); //increase charge if not maxed. 
				else
					charge = maxCharge;
	...
	public void Shoot(Player player, ChargeModPlayer modPlayer)
	{
		...
		if (Item.useAmmo != AmmoID.None)
			player.PickAmmo(Item, out type, out speed, out damage, out knockBack, out usedAmmoItemId, true); //doesnt comsume ammo
		else {
			type = Item.shoot;
		...
		if (ammo.consumable && !player.IsAmmoFreeThisShot(Item, ammo, type))
	...
	{
		if (charge < chargeAmount / 4)
			return; //ensures modifer work properly for tooltips
		float percentCharged = (float)charge / ChargeModPlayer.DefaultCharge;
		if (percentCharged == 1)
			percentCharged += 0.25f;
