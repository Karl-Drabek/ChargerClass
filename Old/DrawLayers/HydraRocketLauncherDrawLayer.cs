...
namespace ChargerClass.Old.DrawLayers;

public class HydraRocketLauncherDrawLayer : AnimatedDrawLayer
{
	int framesAtShoot = 0;
	int shotsAtShoot = 0;
	...
	public override void SafeSetStaticDefaults()
	{
		...
		ticsPerFrame = 10;
		scale = 1f;
		textureAsset = ModContent.Request<Texture2D>("ChargerClass/Common/DrawLayers/HydraRocketLauncherDrawLayer");
		itemType = ModContent.ItemType<HydraRocketLauncher>();
	}
	...
	public override void PreDraw(ref PlayerDrawSet drawInfo)
	{
		...
		if (chargeWeapon.ShotsRemaining > 0 && drawInfo.drawPlayer.itemAnimation == drawInfo.drawPlayer.itemAnimationMax - 1)
			shotsAtShoot = chargeWeapon.ShotsRemaining; //if just shot store shotcount
