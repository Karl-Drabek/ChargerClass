...
public class ConsumingLensLaser : LaserProjectile
{
	public override void SafeSetDefaults()
	{
		...
		TextureAsset = ModContent.Request<Texture2D>(
			"ChargerClass/Content/Projectiles/ConsumingLensLaser"
		);
	}
	...
	{
		Vector2 origin = player.Center - new Vector2(endsWidth, endsWidth) / 2;
		...
		for (int i = 0; i < 3; i++)
		{
			Vector2 dustVel =
				...
				* 2f;
			dust = Dust.NewDustDirect(
				origin + Projectile.velocity * Distance,
				endsWidth,
				endsWidth,
				DustID.SolarFlare,
				dustVel.X,
			...
			dust.noGravity = true;
			dust = Dust.NewDustDirect(
				origin + Projectile.velocity * InitialOffset,
				endsWidth,
				endsWidth,
				DustID.SolarFlare,
				dustVel.X,
	...
	{
		ModLightingSystem.Instance.ResetLighting();
	}
