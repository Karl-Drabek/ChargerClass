		...
		for (int i = 0; i < 10; i++) {
			...
			if (posx * posx + posy * posy < distanceSquared) {
				...
				if (Main.rand.NextBool(1, 3))
					Gore.NewGoreDirect(new EntitySource_Parent(Projectile), new Vector2(Projectile.position.X + posx, Projectile.position.Y + posy), default, Main.rand.Next(61, 64), Main.rand.NextFloat(1f, 1.5f));
