...
public abstract class ChargeWeaponHoldout : ModProjectile
{
    ...
    public float GetChargeAmount(Player player) => player.GetModPlayer<ChargeModPlayer>().GetChargeAmountModifier().ApplyTo(((ChargedWeapon)player.HeldItem.ModItem).chargeAmount);
    ...
    public sealed override void AI() {
        ...
        chargedWeapon.heldProj = Projectile.whoAmI;

        if(Projectile.owner == Main.myPlayer) { //this stuff only works for the projectiles owner
            if(player.noItems || player.CCed) Projectile.Kill(); // Cursed (player.noItems), "Crowd Controlled" (the Frozen debuff).
            if(!player.channel){ //shooting
                if(--Timer <= 0){
                    if(--Shots < 0) Projectile.Kill();
                    else Shoot(player, modPlayer, heldItem, rrp);
                ...
                if(Charge < maxCharge) Charge += 300 / CombinedHooks.TotalUseTime(chargeWeapon.Item.useTime, player, player.HeldItem);
    ...
    public void Shoot(Player player, ChargeModPlayer modPlayer, Item item, Vector2 pos){
        ...
        float chargeSpeed = (float)Math.Clamp((float)Charge / ChargeModPlayer.DefaultCharge, 0.5, 1.0) * item.shootSpeed;
        ...
        ChargedShoot(
            ...
            pos, 
            velocity, 
            chargedWeapon.lastConsumedAmmoType, 
    ...
    private void ChargedShoot(Player player, ChargeModPlayer modPlayer, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback, Item item, int chargeLevel, ChargedWeapon chargedWeapon){
        ...
        type = chargedWeapon.lastConsumedAmmoType;
