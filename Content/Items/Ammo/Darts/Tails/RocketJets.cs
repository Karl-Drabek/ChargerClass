...
public class RocketJets : DartComponent
{
	...
	{
		float modifier = 1 + projectile.ai[2] / 100;
		modifiers.FinalDamage *= modifier;
		modifiers.Knockback *= modifier;
		projectile.CritChance += (int)projectile.ai[2] / 500;
	}
	...
	public override void AI(Projectile projectile, int payloadType)
	{
		...
		Dust.NewDustPerfect(
			projectile.Center,
			DustID.Torch,
			particleVect.RotatedByRandom(MathHelper.ToRadians(10)) * Main.rand.NextFloat(1f, 1.1f),
			100,
			default,
			1f
		);
