...
class DartAssemblyState : UIState
{
	...
	public override void Update(GameTime gameTime)
	{
		...
		{ //otherise update the dart based on the components
			...
			dartResultSlot.Item = new Item(ItemID.None);
		if (dartTailSlot.Item.stack == 0)
			dartTailSlot.Item = new Item(ItemID.None);
		if (dartPayloadSlot.Item.stack == 0)
			dartPayloadSlot.Item = new Item(ItemID.None);
		if (dartTipSlot.Item.stack == 0)
			dartTipSlot.Item = new Item(ItemID.None);
		if (dartResultSlot.Item.stack == 0)
			dartResultSlot.Item = new Item(ItemID.None);
		oldItem = dartResultSlot.Item.Clone();
	}
